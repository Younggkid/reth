...
use reth_storage_api::{HashedPostStateProvider, StateProofProvider, StateRootProvider};
...
struct Args {
    ...
    /// Time the four steps inside the witness pass separately instead of as one call.
    ///
    /// Reproduces the body of `into_execution_witness` with timers between steps, and verifies
    /// once per block that it yields a byte-identical witness to the real call.
    #[arg(long)]
    breakdown: bool,
    ...
    /// (read-only) or silently runs a healing unwind (read-write) — neither of which you want while
...
struct Sample {
    ...
    /// One entry per `--repeat`, only under `--breakdown`.
    t_steps: Vec<WitnessSteps>,
    t_encode: Duration,
        ...
        self.t_witness
            ...
            .map(|(index, elapsed)| {
                serde_json::json!({
                    ...
                    // Present only under `--breakdown`; these sum to roughly `us_witness`.
                    "us_w_codes": self.t_steps.get(index).map(|s| us(s.codes)),
                    "us_w_hash": self.t_steps.get(index).map(|s| us(s.hash)),
                    "us_w_expand": self.t_steps.get(index).map(|s| us(s.expand)),
                    "us_w_trie": self.t_steps.get(index).map(|s| us(s.trie)),
                    "us_w_headers": self.t_steps.get(index).map(|s| us(s.headers)),
                    "us_w_steps_total": self.t_steps.get(index).map(|s| us(s.total())),
...
fn main() -> eyre::Result<()> {
    ...
    let access = match (args.commit, args.allow_inconsistent) {
        ...
        (true, true) => AccessRights::RwInconsistent,
...
/// The four steps inside one witness pass, timed separately.
///
/// `into_execution_witness` is a single call, so getting this breakdown means reproducing its body
/// (`crates/revm/src/witness.rs:26`) with timers between the steps. That is only trustworthy if it
/// stays in lockstep with the original, which is why `--breakdown` also runs the real call once per
/// block and refuses to continue if the two witnesses differ.
#[derive(Debug, Default, Clone, Copy)]
struct WitnessSteps {
    /// Collecting bytecode preimages from the execution cache and the bundle state. Pure memory.
    codes: Duration,
    /// Hashing every touched address and slot into the witness target, and collecting the
    /// preimages. Pure CPU — keccak, once per touched account and once per touched slot.
    hash: Duration,
    /// `hashed_post_state(&bundle_state)`: the provider expands untouched slots of destroyed
    /// accounts into explicit zero writes, which the execution cache does not carry.
    expand: Duration,
    /// The trie walk — `StateProofProvider::witness`. This is the part that reads the database.
    trie: Duration,
    /// Ancestor headers for `BLOCKHASH`, plus RLP-encoding each one.
    headers: Duration,
}

impl WitnessSteps {
    fn total(&self) -> Duration {
        self.codes + self.hash + self.expand + self.trie + self.headers
    }
}

/// Reproduces `ExecutionWitnessRecord::into_execution_witness` with a timer around each step.
///
/// Mirrors `crates/revm/src/witness.rs:26-91` exactly, including the order of operations: the
/// hashed post state from the cache is built first and the provider's expansion is `extend`ed onto
/// it last, so the bundle's final values win.
fn witness_with_breakdown<DB>(
    state_db: &reth_revm::State<DB>,
    state_provider: &(impl HashedPostStateProvider + StateProofProvider + ?Sized),
    headers_provider: &Factory,
    number: u64,
) -> eyre::Result<(alloy_rpc_types_debug::ExecutionWitness, WitnessSteps)> {
    let mut steps = WitnessSteps::default();

    let start = Instant::now();
    let codes: Vec<_> = state_db
        .cache
        .contracts
        .values()
        .map(|code| code.original_bytes())
        .chain(state_db.bundle_state.contracts.values().map(|code| code.original_bytes()))
        .collect();
    steps.codes = start.elapsed();

    let start = Instant::now();
    let mut hashed_state = HashedPostState::default();
    let mut keys: Vec<Bytes> = Vec::new();
    for (address, account) in &state_db.cache.accounts {
        ...
        let storage =
            hashed_state.storages.entry(hashed_address).or_insert_with(|| HashedStorage::new(false));
    ...
    steps.hash = start.elapsed();

    let start = Instant::now();
    hashed_state.extend(state_provider.hashed_post_state(&state_db.bundle_state)?);
    steps.expand = start.elapsed();

    let start = Instant::now();
    let state = state_provider.witness(Default::default(), hashed_state, WITNESS_MODE)?;
    steps.trie = start.elapsed();

    let start = Instant::now();
    let lowest = state_db.block_hashes.lowest().map(|(block_number, _)| block_number);
    let range = lowest.unwrap_or_else(|| number.saturating_sub(1))..number;
    let headers = headers_provider
        .headers_range(range)?
        .into_iter()
        .map(|header| {
            let mut buf = Vec::new();
        ...
        .collect();
    steps.headers = start.elapsed();

    Ok((
        alloy_rpc_types_debug::ExecutionWitness { state, codes, keys, headers, ..Default::default() },
        steps,
    ))
}
...
fn advance_stage_checkpoints(
    ...
    for stage in StageId::ALL {
        ...
        provider.save_stage_checkpoint(stage, StageCheckpoint { block_number: number, ..checkpoint })?;
        advanced.push(stage);
    }
...
fn measure(
    ...
    let mut t_steps = Vec::with_capacity(args.repeat as usize);
    // Populated only under `--breakdown`: the authoritative single-call witness, generated once and
    // compared against the decomposed one to prove the decomposition did not drift.
    let mut reference: Option<eyre::Result<_>> = None;
    let mut t_reference = Duration::ZERO;
    let start = Instant::now();
    let output = executor.execute_with_state_closure(&block, |statedb| {
        if args.breakdown && reference.is_none() {
            ...
            t_reference = started.elapsed();
        }

        for _ in 0..args.repeat {
            let started = Instant::now();
            let generated = if args.breakdown {
                match witness_with_breakdown(statedb, &state, factory, number) {
                    Ok((generated, steps)) => {
                        t_steps.push(steps);
                        Ok(generated)
                    }
            ...
            shapes.push(generated.as_ref().ok().map(|w| {
                (w.state.len(), w.codes.len(), w.keys.len(), w.headers.len())
            }));
    ...
    sample.t_steps = t_steps;
    // The reference call is bookkeeping, not a phase a builder pays for, so it never lands in
    // `t_exec` either.
    let reference = reference.transpose()?;
    sample.t_exec = start.elapsed().saturating_sub(t_witness_total).saturating_sub(t_reference);
    ...
    // The decomposition is only worth anything if it still matches what the shipped call produces.
    //
    // Compared as multisets, not sequences. `state`, `codes` and `keys` are collected from hash-map
    // iteration and `ExecutionWitnessMode::Legacy` never sorts them — only `Canonical` does — so
    // their order is arbitrary and differs between two calls on the same state. `headers` is a
    // block range and *is* ordered, so it is compared as-is.
    if let Some(reference) = reference {
        let sorted = |v: &Vec<alloy_primitives::Bytes>| {
            let mut v = v.clone();
            v.sort_unstable();
            v
        };
        let mismatch = if sorted(&reference.state) != sorted(&witness.state) {
            Some("state")
        } else if sorted(&reference.codes) != sorted(&witness.codes) {
            Some("codes")
        } else if sorted(&reference.keys) != sorted(&witness.keys) {
            Some("keys")
        } else if reference.headers != witness.headers {
            Some("headers")
        } else {
            None
        };
        ...
        if let Some(field) = mismatch {
            bail!(
                "--breakdown drifted from into_execution_witness at block {number}: `{field}` \
                 differs even as a multiset. The per-step timings describe something other than \
                 the shipped path — reconcile witness_with_breakdown against \
                 crates/revm/src/witness.rs before trusting them."
            );
        }
    }
