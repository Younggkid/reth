...
#[derive(Debug, Parser)]
#[command(author, about = "Drive a reth node through the Engine API and measure debug_executionWitness")]
struct Args {
...
fn main() -> eyre::Result<()> {
    ...
    for number in args.from..=to {
        ...
        let ExecutionPayload::V3(payload) = payload else {
            bail!("block {number} did not convert to an ExecutionPayloadV3; a fork boundary here \
        ...
        if !args.skip_witness {
            for iteration in 0..args.witness_repeat {
                ...
                if iteration == 0 {
                    let len = |key: &str| {
                        witness.get(key).and_then(Value::as_array).map_or(0, Vec::len)
                    };
                    let bytes = |key: &str| {
